...
int	main(void)
{
		...
		for (int i = 0; i < 50; i++)
		{
			a = strlcpy(deststrlcpy, srcstrlcpy, 111);
			b = ft_strlcpy(destftstrlcpy, srcstrlcpy, 111);
			if (a != b)
			{
				printf ("la vrai fonction: i = %d %s",a, deststrlcpy);
				printf ("\nma fonction: i = %d %s",b, destftstrlcpy);
				test = 1;
			}
		}
		if (test)
		...
		char deststrlcat[400] = "je fais un test";
		char destftstrlcat[400] = "je fais un test";
		a = strlcat(deststrlcat, srcstrlcat, 30);
		b = ft_strlcat(destftstrlcat, srcstrlcat, 30);

		/*		printf ("la vrai fonction: %d%s", a, deststrlcat);
				printf ("\nma fonction: %d%s\n", b, destftstrlcat);
		memset(&deststrlcat[15], 0, 8);
		memset(&destftstrlcat[15],0,8);*/
		test = 1;
		
		for (int i = 30; i < 80;i++)
		{
			a = strlcat(deststrlcat, srcstrlcat, i);
			b = ft_strlcat(destftstrlcat, srcstrlcat, i);
			if (a != b)
			{
				printf ("%d%d%s", i,a, deststrlcat);
				printf ("\n%d%d%s\n",i, b, destftstrlcat);
				test = 0;
			}
		}
		...
		char *tab3;
		tab1 = ft_strdup("1234");
		tab2 = ft_strdup("abcde");
		tab3 = ft_strdup("56789");
		...
		/*while (mylist)
		{
				printf("mylist = %s",(char*)mylist->content);
				printf("%p %p\n", mylist, mylist->next);
				mylist = mylist->next;
		}*/
				ft_lstadd_back(&mylist, new);
		listmp = mylist;
			...
			printf("%lu,%lu",sizeof(t_list),sizeof(t_list*));
}
